...
st.sidebar.image("icon/houselogo.jpeg", width=150)
...
if selection=='Home':
    ...
    def house_details(loc,sqft,bhk,bath,prediction):
        st.text(f"{'Location📍'.ljust(15)}: {loc}") #ljust for space allignment
    ...
    if c2.button('💰 Predict Price'):
        ...
        with st.spinner('Predicting....'): #chech the right corner running element 
            time.sleep(1)
